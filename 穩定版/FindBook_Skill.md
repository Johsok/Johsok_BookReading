...
1. 必須操作使用者 Chrome 裡已登入的 ChatGPT 網頁版，不得使用 Codex 本身回答、OpenAI API、其他瀏覽器、其他網頁外掛或本地模板替代 ChatGPT 網頁版輸出。
2. 進入 Chrome 的 ChatGPT 頁面，並啟用 ChatGPT 最高階 pro 的 model；多工時依 `chatgptMaxWorkers` 開啟可穩定操作的獨立 Chrome 分頁或對話。
3. ChatGPT 輸入框必須 Key 入該書的固定關鍵字：`書名、作者、標題格式為，01、02、....100，請輸出書籍 100 個重點整理「依照網頁版風格整理」`，其中 `書名` 與 `作者` 要替換成實際書名與作者。
4. 取得 Chrome ChatGPT 網頁版回覆後，檢查是否剛好 100 點。
5. 若不足、超過或編號錯誤，必須在 Chrome ChatGPT 網頁版同一對話要求補齊、刪減或重排到剛好 100 點。
6. 將 Chrome ChatGPT 網頁版最後驗收通過的結果寫回 `index.html` 會讀取的相關 JSON 中，存入該書的 `chatgptHighlights`，並將 `chatgptStatus` 設為 `complete`。
7. 寫入 JSON 時只保留 `01、` 到 `100、` 的重點行；前言、結語、Markdown 標題、清單說明、模型自述或其他多餘文字一律不得寫入。
8. 多本書同時整理時，依 `chatgptMaxWorkers` 平行處理；每本書使用獨立提示與獨立結果暫存，避免內容交叉污染。
...
4. 不得使用 `001_`、`001、`、`01.`、`1、` 或其他非 `01、` 到 `100、` 的編號格式。
5. 任一行包含 `｜`，或在正文開頭使用短標籤加冒號，視為不合格。
6. 任一行不是完整重點句、只是分類名稱、只是提示詞欄位，或包含模型自述、前言、結語，視為不合格。
7. 驗收只檢查格式與來源，不得由 Codex 改寫重點正文。
8. 若輸出不合格，只能回到對應網頁版模型要求重寫不合格的行或整份 100 點；不得把不合格文字直接寫入 JSON。
