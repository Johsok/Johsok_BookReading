...
def load_list(name: str) -> list[str]:
    text = (TOOLS / "_gen_highlights_35_38.py").read_text(encoding="utf-8")
...
def complete(book_id: str, bodies: list[str]) -> None:
    ...
    print(
        f"written/{book['id']}/{len(book['chatgptHighlights'])}/{book.get('highlightsSource')}"
    )
...
if __name__ == "__main__":
    which = sys.argv[1]
    mapping = {
        "35": ("01_business_startup-20260716-35", "BOOK_35"),
        "36": ("01_business_startup-20260716-36", "BOOK_36"),
        "37": ("01_business_startup-20260716-37", "BOOK_37"),
        "38": ("01_business_startup-20260716-38", "BOOK_38"),
    }
    book_id, list_name = mapping[which]
    bodies = load_list(list_name)
    if which == "36" and len(bodies) == 151:
        bodies = bodies[:150]
