...
4. 書名下拉選單每頁最多顯示 80 筆，上一頁、頁碼、下一頁整合在書名控制內。
5. 搜尋結果也遵守每頁 80 筆規則。書名後方附上出版年份，例如「原子習慣 (2024年)」。
...
8. 可從左側「刪」或工具列「刪除本書」將書籍從 `data.json` 索引移除（需確認）；瀏覽器無法刪除站內檔案，儲存後需手動刪除對應 `Books/.../*.json`。
